...
public class HeapPorGananciaConHandle<T> {
    private ArrayList<Handle> heap;
    private int tamaño;
    private Handle handle;
    private Comparador<T> comparador;

    public HeapPorGananciaConHandle(ArrayList<Traslado> array, Comparador<T> comparador) {
        this.heap = new ArrayList<>();  // Aquí no usamos copyOf
        this.comparador = new Comparador<Traslado>(comparador);
        MaxHeap();
        tamaño = 0;
        for(int i=0; i < array.size(); i++){
            heap.add(new Handle(array.get(i), i, -1));
        }
    }
    ...
    private void heapify(int i) {
        ...
        if (posicionIzq < heap.size() && comparador.comp(izq, mayor) > 0) {
            mayor = izq;
    ...
    public void agregar(Traslado tras){
        Handle h = new Handle(tras, tamaño-1, -1);
        heap.set(tamaño,h);
        Handle padre = heap.get((tamaño-1) / 2);
        int posActual = tamaño;
        int posPadre = (tamaño-1) / 2;

        while (tras.gananciaNeta > padre.traslado.gananciaNeta){
            heap.set(posPadre, tras, -1);
